...
class PairRepositoryImpl implements PairRepository {
  ...
  Future<PairEntity?> getPairById(String id) async {
    try {
      ...
      throw FirestoreException(message: e.message ?? 'Failed to get pair', code: e.code);
    }
  }
  ...
  Future<PairEntity> createPair(PairEntity pair) async {
    try {
      final docRef = await _pairsCollection.add(PairModel.fromEntity(pair).toMap());
      return pair.copyWith(id: docRef.id);
    } on FirebaseException catch (e) {
      throw FirestoreException(message: e.message ?? 'Failed to create pair', code: e.code);
    }
  }
  ...
  Future<void> updatePair(PairEntity pair) async {
    try {
      await _pairsCollection.doc(pair.id).update(PairModel.fromEntity(pair).toMap());
    } on FirebaseException catch (e) {
      throw FirestoreException(message: e.message ?? 'Failed to update pair', code: e.code);
    }
  }
  ...
  Future<PairInviteEntity> createInvite(PairInviteEntity invite) async {
    try {
      final docRef = await _invitesCollection.add(PairInviteModel.fromEntity(invite).toMap());
      return invite.copyWith(id: docRef.id);
    } on FirebaseException catch (e) {
      throw FirestoreException(message: e.message ?? 'Failed to create invite', code: e.code);
    }
  }
  ...
  Future<List<PairInviteEntity>> getInvitesForEmail(String email) async {
    try {
      final snapshot = await _invitesCollection.where('toEmail', isEqualTo: email).get();
      return snapshot.docs
          .map((doc) => PairInviteModel.fromMap(doc.data(), doc.id).toEntity())
          .toList();
    } on FirebaseException catch (e) {
      throw FirestoreException(message: e.message ?? 'Failed to get invites', code: e.code);
    }
  }
  ...
  Future<void> updateInviteStatus(String inviteId, String status) async {
    try {
      ...
      throw FirestoreException(message: e.message ?? 'Failed to update invite', code: e.code);
    }
  }
